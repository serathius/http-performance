...
benchmark_1_short:
	docker run --rm --net=host --cpuset-cpus="1" 1vlad/wrk2-docker -t1 -c10 -d30s -R100 --latency http://127.0.0.1:8000 | grep 'Latency Distribution\|Requests' -A 8 | awk '{print $$2}' | sed 's/[^0-9^.]*//g'
...
benchmark_2_short:
	docker run --rm --net=host --cpuset-cpus="1" 1vlad/wrk2-docker -t1 -c100 -d30s -R1000 --latency http://127.0.0.1:8000 | grep 'Latency Distribution\|Requests' -A 8 | awk '{print $$2}' | sed 's/[^0-9^.]*//g'
...
benchmark_3_short:
	docker run --rm --net=host --cpuset-cpus="1,2" 1vlad/wrk2-docker -t2 -c1000 -d300s -R10000 --latency http://127.0.0.1:8000 | grep 'Latency Distribution\|Requests' -A 8 | awk '{print $$2}' | sed 's/[^0-9^.]*//g'
